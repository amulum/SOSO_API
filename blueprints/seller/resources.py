...
class SellerCreateTokenResource(Resource):
    ...
    def post(self):
        ...
        qry = Seller.query
        selected_seller = qry.filter_by(username=args['username']).filter_by(
            password=password_digest).first()
        if selected_seller:
            token = create_access_token(identity='VERIFIED_SELLER', user_claims={
                                        'id': selected_seller.id, 'username': args['username']})
    ...
    def post(self):
        ...
        if args['username'][-3] != '-VS':
            ...
            if validation == []:
                ...
                # Creating object
                new_seller = Seller(
                    brand_name=args['brand_name'], username=args['username'], password=password_digest, email=args['email'])
                print(new_seller)
        ...
        return {'status': 'FAILED', 'message': 'PLEASE ENTER USERNAME WITH SPECIFIC FORMAT'}, 400
...
class SellerProduct(Resource):
    ...
    def get(self):
        ...
        parser.add_argument("order_by", location="json",
                            help="invalid order-by value", choices=("id", "price"))
        ...
        if args["order_by"] == "id":
            ...
            if args["sort"] == "desc":
                qry = qry.order_by(desc(Product.sell_price))
            else:
                qry = qry.order_by(Product.sell_price)
        ...
        if lst_product:
            ...
            for index in range(len(lst_product)):
                marshal_product = (
                    marshal(lst_product[index], Product.seller_fields))
                format_product = {
                    'Product': index+1,
    ...
    def patch(self):
        ...
        qry = Product.query
        selected_product = qry.filter_by(id=args['id']).filter_by(
            seller_id=claims['id']).first()
        if selected_product:
            ...
            if args['discount']:
                ...
                selected_product.sell_price = int(
                    selected_product.price) * (100-int(args['discount']))/100
            else:
                selected_product.sell_price = int(
                    selected_product.price) * (100-int(selected_product.discount))/100
            db.session.commit()
...
class SellerStockProduct(Resource):
    ...
    def post(self):
        ...
        if selected_product and selected_stock == [] and int(args['qty_onupdate']) > 0:
            ...
            selected_stock.current_qty = int(
                selected_stock.current_qty) + int(args['qty_onupdate'])
            db.session.commit()
            return {
                'message': 'Add Stock Succesful',
