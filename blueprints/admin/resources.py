    ...
    def post(self):
        ...
        if selected_admin:
            if selected_admin.password == password_digest:
                ...
                db.session.commit()
                token = create_access_token(identity='ADMIN', user_claims={
...
class AdminResourceForSeller(Resource):
    ...
    def get(self):
        ...
        if args['id'] or args['username'] or args['brand_name']:
            if args['id']:
                selected_seller = qry.filter_by(id=args['id']).first()
            if args['username']:
                selected_seller = qry.filter_by(
                    username=args['username']).first()
            if args['brand_name']:
                selected_seller = qry.filter_by(
                    brand_name=args['brand_name']).first()
            if selected_seller:
...
class AdminResourceForCust(Resource):
    ...
    def get(self):
        ...
        if args['id'] or args['username']:
            if args['id']:
                selected_customer = qry.filter_by(id=args['id']).first()
            if args['username']:
                selected_customer = qry.filter_by(
                    username=args['username']).first()
        ...
        if lst_cust:
            ...
            for each_cust in lst_cust:
                customer_result.append(
                    marshal(each_cust, Customer.admin_fields))
    ...
    def delete(self):
        ...
        qry = Customer.query
        if args['id']:
            selected_cust = qry.filter_by(id=args['id']).first()
        if args['username']:
            selected_cust = qry.filter_by(username=args['username']).first()
        if selected_cust:
...
class AdminResourceForPopular(Resource):
    ...
    def get(self):
        ...
        # reference query
        dum = db.session.query(MyBagDetails.product_id, db.func.count(MyBagDetails.id)).group_by(
            MyBagDetails.product_id).order_by(desc(db.func.count(MyBagDetails.id))).first()

        count_product = db.func.count(MyBagDetails.id)
        product_id = MyBagDetails.product_id
        qry = db.session.query(product_id, count_product).group_by(
            product_id).order_by(desc(count_product)).first()
        result_pid = qry[0]
